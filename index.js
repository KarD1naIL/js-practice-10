...
function customObjectFromEntries(entries) {
  ...
  let resultObject = Object.fromEntries(processedEntries);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (Object.isFrozen(obj1)) {
    ...
    return "Помилка: Об'єкт не вдалося заморозити.";
  }
...
function filterObjectsByKey(arr, key) {
  ...
  let filteredArray = arr.filter((obj) => obj.hasOwnProperty(key));
...
function checkAndPreventExtensions(obj) {
  ...
  let canAddProperties = Object.isExtensible(obj);
...
function convertArrayToObj(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (resultObj.hasOwnProperty(key)) {
      console.log(`У масиві є дублюючий ключ: ${key}`);
    }
